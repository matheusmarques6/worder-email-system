...
export async function POST(request: NextRequest) {
  try {
    ...
    if (form.status === "inactive") {
      return NextResponse.json(
        { success: false, error: "Form is inactive" },
    ...
    if (contactError) {
      ...
      return NextResponse.json(
        { success: false, error: "Error processing contact" },
    ...
    await supabase
      .rpc("increment_form_submissions", {
        p_form_id: form_id,
      })
      .then(async (rpcResult) => {
        if (rpcResult.error) {
          ...
          const currentCount =
            (currentForm?.submissions_count as number) ?? 0;
          await supabase
            .from("forms")
